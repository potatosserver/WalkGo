...
**目標：** 統一全應用程式彈窗（Dialog）的形狀與大小，提升視覺一致性。

### 實作步驟

1.  **全域主題配置**：
    *   在 `ThemeData` 中定義 `dialogTheme`。
    *   統一設定圓角半徑（例如 $24\text{dp}$）。
    *   統一標題與內容的文字樣式。
2.  **清理硬編碼約束**：
    *   修改 `lib/widgets/update_dialog.dart` 與 `lib/widgets/release_notes_dialog.dart`。
    *   移除固定的 `maxWidth: 500`，改為響應式寬度（手機端 $\approx 90\%$，平板/Web 端設上限）。
3.  **視覺校驗**：
    *   檢查確認類、資訊類、功能類所有彈窗的顯示效果。
